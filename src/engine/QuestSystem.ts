...
const QUEST_TEMPLATES: Omit<Quest, 'id' | 'progress' | 'completed'>[] = [
  {
    name: 'Monster Hunter',
    description: 'Defeat 5 cave skeletons',
    icon: '💀',
    objective: QuestObjective.KILL,
    target: 'skeleton',
    required: 5,
    rewards: [{ itemId: 'iron_ingot', count: 5 }],
    xpReward: 50,
  },
  {
    name: 'Spider Slayer',
    description: 'Defeat 10 cave spiders',
    icon: '🕷️',
    objective: QuestObjective.KILL,
    target: 'spider',
    required: 10,
    rewards: [{ itemId: 'string', count: 20 }],
    xpReward: 60,
  },
  {
    name: 'Iron Miner',
    ...
    target: 'iron_ore',
    required: 20,
    rewards: [{ itemId: 'gold_ingot', count: 5 }],
    xpReward: 40,
  },
  {
    name: 'Diamond Seeker',
    description: 'Mine 3 diamonds',
    icon: '💎',
    objective: QuestObjective.MINE,
    target: 'diamond',
    required: 3,
    rewards: [{ itemId: 'crystal', count: 5 }],
    xpReward: 80,
  },
  {
    name: 'Master Smith',
    ...
    icon: '🗡️',
    objective: QuestObjective.CRAFT,
    target: 'weapon',
    required: 5,
    rewards: [{ itemId: 'diamond', count: 2 }],
    xpReward: 60,
  },
  {
    name: 'Trader',
    description: 'Complete 3 trades with villagers',
    icon: '🤝',
    objective: QuestObjective.TRADE,
    target: 'any',
    required: 3,
    rewards: [{ itemId: 'gold_ingot', count: 8 }],
    xpReward: 40,
  },
  {
    name: 'Explorer',
    description: 'Discover 3 structures',
    icon: '🗺️',
    objective: QuestObjective.EXPLORE,
    target: 'any',
    required: 3,
    rewards: [{ itemId: 'crystal', count: 3 }],
    xpReward: 50,
  },
  {
    name: 'Wave Survivor',
    description: 'Survive 5 enemy waves',
    icon: '⚔️',
    objective: QuestObjective.SURVIVE_WAVES,
    target: 'any',
    required: 5,
    rewards: [{ itemId: 'diamond', count: 3 }],
    xpReward: 100,
  },
  {
    name: 'Farmer',
    ...
    target: 'any',
    required: 10,
    rewards: [{ itemId: 'golden_apple', count: 3 }],
    xpReward: 40,
  },
  {
    name: 'Gold Rush',
    description: 'Mine 10 gold ore',
    icon: '🟡',
    objective: QuestObjective.MINE,
    target: 'gold_ore',
    required: 10,
    rewards: [{ itemId: 'diamond', count: 1 }],
    xpReward: 50,
  },
];
...
export class QuestSystem {
  ...
  private generateQuests(): void {
    while (this.activeQuests.length < this.maxActive) {
      const available = QUEST_TEMPLATES.filter(
        (t) => !this.completedIds.has(t.name) && !this.activeQuests.some((q) => q.name === t.name),
      );
  ...
  private wireEvents(): void {
    ...
    eventBus.on(Events.BLOCK_DESTROYED, (data: any) => {
      const oreNames: Record<number, string> = {
        30: 'coal',
        31: 'iron_ore',
        32: 'gold_ore',
        33: 'crystal',
