...
import {
  FOV, NEAR_PLANE, FAR_PLANE, AMBIENT_LIGHT_INTENSITY, DIRECTIONAL_LIGHT_INTENSITY,
  WORLD_SEED, BASE_PLAYER_HP, CHUNK_SIZE,
...
multiplayerMenu.setConnectedCallback(() => {
  ...
  if (networkManager.isHost) {
    networkManager.send({ kind: NetMsgKind.WORLD_SEED, seed: WORLD_SEED, enemyCastleX: ecX, enemyCastleZ: ecZ });
  }
});
  ...
  switch (msg.kind) {
    ...
    case NetMsgKind.COMBAT_EVENT:
      if (msg.eventType === 'damage' && msg.pos) {
        eventBus.emit(Events.ENTITY_DAMAGED, { entityId: msg.entityId, damage: msg.damage, remaining: msg.remaining, pos: msg.pos });
    ...
    case NetMsgKind.WORLD_SEED: {
      ...
      console.log(`[MP] Received world seed: ${msg.seed}, enemy castle: (${msg.enemyCastleX}, ${msg.enemyCastleZ})`);
    ...
    case NetMsgKind.GAME_OVER: {
      ...
      if (msg.result === 'victory') {
        const mins = Math.floor(msg.time / 60), secs = Math.floor(msg.time % 60);
        overlay.innerHTML = `
...
// Player's castle
const playerCastle = new Castle('player_castle', 'player', spawnX + 10, spawnHeight + 1, spawnZ + 10);
...
pauseMenu.setCallbacks(
  () => { /* resume */ },
  () => { saveSystem.save(player, playerHealth, inventory, playerCastle, enemyCastle, gameManager.isEnemyCastleDiscovered, gameTime, dayNight.time); hud.showInfo('💾 Saved!', 2000); },
  () => { const d = saveSystem.load(); if (d) { const a = saveSystem.apply(d, player, inventory); playerHealth = a.health; dayNight.time = a.dayTime; hud.showInfo('💾 Loaded!', 2000); } },
);

import { Entity, createTeam, createPosition, createHealth } from './ecs';
const playerEntity = new Entity('player-entity');
playerEntity.addComponent(createTeam('player'))
...
eventBus.on(Events.CHUNK_LOADED, () => {
  if (!enemyCastlePlaced) {
    const cx = Math.floor(ecX / CHUNK_SIZE), cz = Math.floor(ecZ / CHUNK_SIZE);
    if (chunkManager.getChunk(cx, cz)) { enemyGen.placeBlocks(chunkManager, enemyCastle); enemyCastlePlaced = true; }
  }
});
...
waveManager.onSpawn((type, count) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    warriorManager.spawnWarrior(type, 'enemy', sx, spawnHeight + 1, sz, enemyCastle.id, playerCastle.id);
  }
});
...
eventBus.on(Events.BLOCK_DESTROYED, (data: unknown) => {
  ...
  if (d.x !== undefined && d.y !== undefined && d.z !== undefined) {
    if (d.projectile) { particles.emitExplosion(d.x, d.y, d.z); soundManager.playExplosion(); }
...
eventBus.on(Events.ARROW_FIRED, (data: any) => {
  projectiles.launchArrow(data.from.x, data.from.y, data.from.z, data.to.x, data.to.y, data.to.z, data.damage, data.team);
...
tutorial.setSteps([
  { id: 'look', text: 'Click to start, then look around with the mouse', icon: '🖱️', condition: () => input.isPointerLocked },
  { id: 'move', text: 'Press WASD to walk around', icon: '🚶', condition: () => player.x !== spawnX || player.z !== spawnZ },
  { id: 'mine', text: 'Hold Left Click on a block to mine it', icon: '⛏️', condition: () => tutorialBlocksMined >= 1 },
  { id: 'mine_more', text: 'Great! Mine 5 more blocks for materials', icon: '💎', condition: () => tutorialBlocksMined >= 6 },
  { id: 'craft', text: 'Press C to open the crafting menu', icon: '⚒️', condition: () => tutorialItemsCrafted >= 1 },
  { id: 'place', text: 'Right Click to place a block', icon: '🧱', condition: () => tutorialBlocksPlaced >= 1 },
  { id: 'castle', text: 'Press B to build castle structures!', icon: '🏰', condition: () => playerCastle.buildings.length > 5 },
]);
...
// Crafting callback
craftingUI.setCraftCallback((recipe) => { crafting.craft(recipe, inventory); hud.showInfo(`⚒️ ${recipe.name}`); });

// ── Block Interaction & Combat ───────────────────────────────────
const damageNumbers = new DamageNumbers(camera);
const raycaster = new THREE.Raycaster();
raycaster.far = 6;
let miningTarget: { x: number; y: number; z: number; block: BlockType; progress: number } | null = null;
let clickCooldown = 0;
...
function checkPlayerMeleeAttack(): boolean {
  ...
  for (const entity of entities) {
    ...
    if (dist <= attackRange) {
      ...
      if (dot > 0.8) { // Approx 36 degree cone
        hitEntity = entity;
        minPos = pos;
        break; 
      }
...
function updateBlockInteraction(dt: number): void {
  if (!input.isPointerLocked || craftingUI.isVisible || pauseMenu.isVisible) { blockHighlight.hide(); return; }
  raycaster.setFromCamera(new THREE.Vector2(0, 0), camera);
  const meshes = scene.children.filter((c): c is THREE.Mesh => c instanceof THREE.Mesh);
  const intersects = raycaster.intersectObjects(meshes);
  if (intersects.length === 0) { blockHighlight.hide(); miningTarget = null; return; }
  const hit = intersects[0];
  const normal = hit.face?.normal;
  if (!normal || !hit.point) { blockHighlight.hide(); return; }
  ...
  if (block === BlockType.AIR) { blockHighlight.hide(); miningTarget = null; hud.updateCrosshair('default'); return; }
  ...
  if (input.isMouseDown(0)) {
    if (castleBuildUI.isPlacing) { castleBuildUI.placeBuilding(); clickCooldown = 0.3; return; }
    ...
    if (props.hardness <= 0) { miningTarget = null; blockHighlight.hideMiningProgress(); }
    else {
      const sel = inventory.getSelectedItem();
      const toolDef = sel ? getItemDef(sel.itemId) : null;
      ...
      if (miningTarget.progress >= 1.0) {
        ...
        if (networkManager.isConnected) networkManager.send({ kind: NetMsgKind.BLOCK_CHANGE, x: bx, y: by, z: bz, blockType: BlockType.AIR });
    ...
    if (miningTarget) { miningTarget = null; blockHighlight.hideMiningProgress(); }
  }
  // Placing / Interacting
  if (input.isMouseDown(2) && clickCooldown <= 0) {
    ...
    if (sel) {
      ...
      if (def?.placesBlock) {
        ...
        const dx = px - Math.floor(player.x), dz = pz - Math.floor(player.z), dy = py - Math.floor(player.y);
        if (!(Math.abs(dx) < 1 && Math.abs(dz) < 1 && dy >= 0 && dy < 2)) {
          ...
          if (networkManager.isConnected) networkManager.send({ kind: NetMsgKind.BLOCK_CHANGE, x: px, y: py, z: pz, blockType: def.placesBlock });
          // Add torch light
...
let lastPlayerX = player.x, lastPlayerZ = player.z;
...
document.addEventListener('keydown', (e) => {
  ...
  if (key === 'c') { craftingUI.toggle(); if (craftingUI.isVisible) document.exitPointerLock(); }
  if (key === 'e' && !craftingUI.isVisible) {
    const avail = crafting.getCraftableRecipes(CraftingStation.HAND, inventory);
    if (avail.length > 0) { crafting.craft(avail[0], inventory); hud.showInfo(`⚒️ ${avail[0].name}`); }
  }
  ...
  if (key === 'escape') {
    ...
    else if (deathScreen.isVisible) { /* can't escape death */ }
  ...
  if (key === 'n' && !craftingUI.isVisible && !pauseMenu.isVisible && !inventoryUI.isVisible) multiplayerMenu.toggle();
  if (key === 'i' && !craftingUI.isVisible && !pauseMenu.isVisible) inventoryUI.toggle();
  if (key === 'f5') { e.preventDefault(); const ok = saveSystem.save(player, playerHealth, inventory, playerCastle, enemyCastle, gameManager.isEnemyCastleDiscovered, gameTime, dayNight.time); hud.showInfo(ok ? '💾 Saved!' : '❌ Failed', 2000); }
...
function gameLoop(): void {
  ...
  const paused = pauseMenu.isVisible || deathScreen.isVisible || scoreDashboard.isVisible || inventoryUI.isVisible;
  ...
  if (!craftingUI.isVisible && !paused) {
    ...
    const pPos = playerEntity.getComponent<import('./ecs/Component').PositionComponent>('position')!;
    pPos.x = player.x; pPos.y = player.y + 1.0; pPos.z = player.z;
  ...
  if (!paused) {
    ...
    // Castle build ghost
    if (castleBuildUI.isPlacing) castleBuildUI.updateGhost(player.x, player.y, player.z, player.yaw);

    // Achievements — track movement
    const dx = player.x - lastPlayerX, dz = player.z - lastPlayerZ;
  ...
  // Sun follows player
  sunLight.position.x = player.x + 100; sunLight.position.z = player.z + 50;
  ...
  const biomeLabel: Record<string, string> = { forest: '🌲 Forest', plains: '🌾 Plains', desert: '🏜️ Desert', tundra: '❄️ Tundra', swamp: '🌿 Swamp', mountain: '⛰️ Mountain' };
  ...
  const standingBlock = chunkManager.getBlockAtWorld(Math.floor(player.x), Math.floor(player.y) - 1, Math.floor(player.z));
  ...
  const headBlock = chunkManager.getBlockAtWorld(Math.floor(player.x), Math.floor(player.y + 1.6), Math.floor(player.z));
  ...
  minimap.setEntities(warriors.map((w) => { const pos = w.getComponent<PositionComponent>('position')!; const team = w.getComponent<TeamComponent>('team')!; return { x: pos.x, z: pos.z, color: team.team === 'player' ? '#3498db' : '#e74c3c', size: 2 }; }));
  minimap.update();

  // Auto-save
  autoSaveTimer += dt;
  if (autoSaveTimer >= 60) { autoSaveTimer = 0; saveSystem.save(player, playerHealth, inventory, playerCastle, enemyCastle, gameManager.isEnemyCastleDiscovered, gameTime, dayNight.time); }
...
// ── Resize ───────────────────────────────────────────────────
window.addEventListener('resize', () => { camera.aspect = window.innerWidth / window.innerHeight; camera.updateProjectionMatrix(); renderer.setSize(window.innerWidth, window.innerHeight); postProcessing.resize(window.innerWidth, window.innerHeight); });
...
// eslint-disable-next-line no-console
console.log(`   Seed: ${WORLD_SEED} | Enemy: (${ecX},${ecZ}) ~${Math.sqrt(ecX * ecX + ecZ * ecZ).toFixed(0)}blk`);
// eslint-disable-next-line no-console
console.log(`   Recipes: ${crafting.getAllRecipes().length} | Biomes: 6 | Achievements: 16`);
// eslint-disable-next-line no-console
console.log(`   WASD Move|C Craft|B Build|F Eat|N Multiplayer|F5 Save|F9 Load|M Sound|Tab Skip Tutorial`);
