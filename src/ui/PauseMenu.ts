...
export class PauseMenu {
  ...
  constructor() {
    ...
    this.overlay.style.cssText =
      'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.75);z-index:400;display:none;align-items:center;justify-content:center;flex-direction:column;font-family:monospace;';

    const panel = document.createElement('div');
    panel.style.cssText =
      'background:rgba(30,30,30,0.95);border:2px solid rgba(255,255,255,0.15);border-radius:12px;padding:30px 50px;text-align:center;backdrop-filter:blur(10px);';
    ...
    for (const b of buttons) {
      ...
      btn.addEventListener('mouseenter', () => {
        btn.style.background = `${b.color}33`;
        btn.style.borderColor = b.color;
      });
      btn.addEventListener('mouseleave', () => {
        btn.style.background = 'rgba(255,255,255,0.08)';
        btn.style.borderColor = `${b.color}44`;
      });
    ...
    volDiv.innerHTML =
      '<div style="color:#bdc3c7;font-size:13px;margin-bottom:6px;">🔊 Volume</div>';
    ...
    soundBtn.style.cssText =
      'display:block;width:100%;padding:8px;margin:8px 0 0;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);color:#7f8c8d;font-family:monospace;font-size:13px;border-radius:6px;cursor:pointer;pointer-events:auto;';
