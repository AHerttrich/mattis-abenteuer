...
export class HUD {
  ...
  constructor() {
    ...
    this.container.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:100;font-family:monospace;color:#fff;';
  ...
  private createCrosshair(): void {
    ...
    this.crosshair.style.cssText = 'position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:24px;height:24px;';
  ...
  private createHealthBar(): void {
    ...
    wrapper.style.cssText = 'position:absolute;bottom:60px;left:50%;transform:translateX(-50%);display:flex;gap:6px;align-items:center;';
    const healthWrap = document.createElement('div');
    healthWrap.style.cssText = 'width:145px;background:rgba(0,0,0,0.6);border-radius:6px;padding:4px;position:relative;';
    const heartIcon = document.createElement('span');
    heartIcon.style.cssText = 'position:absolute;top:-1px;left:6px;font-size:10px;';
    heartIcon.textContent = '❤️';
    this.healthBar = document.createElement('div');
    this.healthBar.style.cssText = 'height:12px;background:linear-gradient(90deg,#e74c3c,#e67e22);border-radius:4px;width:100%;transition:width 0.3s;';
    this.healthText = document.createElement('span');
    this.healthText.style.cssText = 'position:absolute;top:2px;left:50%;transform:translateX(-50%);font-size:10px;text-shadow:1px 1px 2px #000;';
  ...
  private createHungerBar(): void {
    const wrapper = document.createElement('div');
    wrapper.style.cssText = 'position:absolute;bottom:60px;left:50%;transform:translateX(-50%);margin-left:155px;';
    const hungerWrap = document.createElement('div');
    hungerWrap.style.cssText = 'width:145px;background:rgba(0,0,0,0.6);border-radius:6px;padding:4px;position:relative;';
    ...
    this.hungerBar.style.cssText = 'height:12px;background:linear-gradient(90deg,#d4a056,#c0792a);border-radius:4px;width:100%;transition:width 0.3s,background 0.3s;';
    this.hungerText = document.createElement('span');
    this.hungerText.style.cssText = 'position:absolute;top:2px;left:50%;transform:translateX(-50%);font-size:10px;text-shadow:1px 1px 2px #000;';
  ...
  private createHotbar(): void {
    ...
    this.hotbar.style.cssText = 'position:absolute;bottom:10px;left:50%;transform:translateX(-50%);display:flex;gap:4px;';
    for (let i = 0; i < 9; i++) {
      const slot = document.createElement('div');
      slot.id = `hotbar-${i}`;
      slot.style.cssText = 'width:44px;height:44px;background:rgba(0,0,0,0.6);border:2px solid rgba(255,255,255,0.3);border-radius:4px;display:flex;align-items:center;justify-content:center;font-size:10px;';
  ...
  private createDebugInfo(): void {
    ...
    this.debugInfo.style.cssText = 'position:absolute;top:10px;left:10px;font-size:12px;text-shadow:1px 1px 2px #000;line-height:1.6;';
    this.container.appendChild(this.debugInfo);
  }

  private createInfoText(): void {
    this.infoText = document.createElement('div');
    this.infoText.style.cssText = 'position:absolute;top:50%;left:50%;transform:translate(-50%,-80px);font-size:18px;text-shadow:2px 2px 4px #000;text-align:center;opacity:0;transition:opacity 0.5s;';
    this.container.appendChild(this.infoText);
  }

  private createDamageFlash(): void {
    this.damageFlash = document.createElement('div');
    this.damageFlash.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:99;opacity:0;transition:opacity 0.08s;background:radial-gradient(ellipse at center, transparent 40%, rgba(200,0,0,0.4) 100%);';
    document.body.appendChild(this.damageFlash);
  }

  private createLowHpOverlay(): void {
    this.lowHpOverlay = document.createElement('div');
    this.lowHpOverlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:98;opacity:0;box-sizing:border-box;';
  ...
  updateHealth(current: number, max: number): void {
    ...
    this.healthBar.style.background = pct > 50
    ...
    if (pct <= 20 && !this.lowHpActive) {
      ...
      this.lowHpOverlay.style.background = 'radial-gradient(ellipse at center, transparent 50%, rgba(180,0,0,0.35) 100%)';
  ...
  updateHunger(current: number, max: number): void {
    ...
    this.hungerBar.style.background = pct > 50
  ...
  updateHotbar(items: ({ name: string; count: number; itemId?: string; durability?: number; maxDurability?: number } | null)[], selected: number): void {
    for (let i = 0; i < 9; i++) {
      ...
      if (item) {
        ...
        label.style.cssText = 'display:flex;flex-direction:column;align-items:center;justify-content:center;width:100%;height:100%;position:relative;';
        // Emoji icon
        const iconSpan = document.createElement('span');
        iconSpan.style.cssText = 'font-size:20px;line-height:1;filter:drop-shadow(0 1px 2px rgba(0,0,0,0.5));';
        iconSpan.textContent = icon;
        label.appendChild(iconSpan);
        // Count badge
        if (item.count > 1) {
          const countBadge = document.createElement('span');
          countBadge.style.cssText = 'position:absolute;bottom:1px;right:2px;font-size:10px;font-weight:bold;text-shadow:1px 1px 2px #000;';
        ...
        if (item.durability !== undefined && item.maxDurability) {
          ...
          barWrap.style.cssText = 'position:absolute;bottom:2px;left:4px;right:4px;height:4px;background:rgba(0,0,0,0.5);border-radius:1px;';
  ...
  updateDebug(fps: number, x: number, y: number, z: number, chunks: number, biome?: string, time?: string, warriors?: number): void {
  ...
  showInfo(text: string, durationMs = 3000): void {
    ...
    setTimeout(() => { this.infoText.style.opacity = '0'; }, durationMs);
  }

  showStartScreen(): void {
    const overlay = document.createElement('div');
    overlay.id = 'start-screen';
    overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:200;cursor:pointer;pointer-events:auto;overflow:hidden;';
    overlay.innerHTML = `
      <style>
  ...
  setUnderwaterOverlay(active: boolean): void {
    if (active && !this.underwaterOverlay) {
      ...
      this.underwaterOverlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:95;background:radial-gradient(ellipse at center,rgba(30,60,120,0.3) 0%,rgba(10,30,80,0.5) 100%);transition:opacity 0.5s;';
  ...
  updateWave(waveNumber: number, phase: string, timer: number, enemiesAlive: number): void {
    if (!this.waveDisplay) {
      ...
      this.waveDisplay.style.cssText = 'position:absolute;top:12px;right:12px;background:rgba(0,0,0,0.6);padding:8px 14px;border-radius:6px;font-size:13px;text-align:right;pointer-events:none;z-index:102;';
  ...
  showWaveBanner(text: string, color: string = '#f1c40f', duration: number = 3000): void {
    ...
    setTimeout(() => { if (this.waveBanner) { this.waveBanner.remove(); this.waveBanner = null; } }, duration);
  }
